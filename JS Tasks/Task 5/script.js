...
function filterDogOwers() {
  for (let i = 0; i < users.length; i++) {
    if (users[i].hasDog == true) {
      console.log(`${users[i].name} turi šunį`);
    }
  }
...
function filterAdults() {
  for (let i = 0; i < users.length; i++) {
    if (users[i].age >= 18) {
      console.log(`${users.name} yra pinametis`)
    }
...
// const filterDogOwers = users.find(function(m){
//   return m=> m.hasDog ===true;
// });
